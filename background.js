function generateMarkdownAndTimestamp(tabs, titleFormat, urlFormat) {
  ...
  for (const tab of tabs) {
    let formattedTitle = titleFormat.replace(/\{title\}/g, tab.title).replace(/\{url\}/g, tab.url).replace(/\\n/g, '\n');
    let formattedUrl = urlFormat.replace(/\{title\}/g, tab.title).replace(/\{url\}/g, tab.url).replace(/\\n/g, '\n');
  ...
  chrome.browserAction.onClicked.addListener(() => {
    chrome.storage.sync.get(['restrictedUrls', 'titleFormat', 'urlFormat'], (result) => {
      ...
      chrome.windows.getAll({ populate: true, windowTypes: ['normal'] }, (windows) => {
        ...
        const { markdown, formattedTimestamp } = generateMarkdownAndTimestamp(tabs, titleFormat, urlFormat);
